    ...
    <div class="container-fluid">
        ...
        <div class="row">
            <div class="col-lg-10">
                <div class="card">
                    <div class="card-body">
                        <form action="{{url('customers/update')}}" method="POST" novalidate enctype="multipart/form-data">
                            @csrf
                            <input type="hidden" name="id" id="uid" value="{{$customer->id}}">
                            <input type="hidden" name="acount" id="ac" value="{{count($addresses) - 1}}">
                            <input type="hidden" name="pcount" id="pc" value="{{count($phones) - 1}}">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control @if($errors->has('last_name')) is-invalid @endif" name="last_name" type="text"
                                       id="last_name" placeholder="Enter your last name" required value="{{ $customer->last_name ?? old('last_name')}}"/>
                            ...
                            <?php /*div class="form-group">
                                ...
                                <a href="javascript:void(0);" id="add-more" data-type="address" class="float-right">Add
                                    More</a>
                                @if(isset($addresses) && count($addresses) > 0)
                                    ...
                                    @foreach($addresses AS $address)
                                        @php
                                            ...
                                            if($a > 0){
                                                $address_name = 'address_'.$a;
                                            }
                                        @endphp
                                        <input class="form-control {{ $a > 0 ? 'mt-2' : '' }}" name="{{$address_name}}" type="text"
                                               id="address" placeholder="Enter your address" required value="{{ $address->address ?? old('address')}}"/>
                                        @php $a++; @endphp
                                    @endforeach
                                @else
                                    <input class="form-control" name="address" type="text"
                                           id="address" placeholder="Enter your address" required value="{{ old('address')}}"/>
                                @endif
                            </div */?>
                            <div class="form-group">
                                <label for="address">Address</label>
                                ...
                                <select class="form-control select2" name="address[]" id="address" data-toggle="select2" multiple>
                                    @if(isset($addresses) && count($addresses) > 0)
                                        @foreach($addresses AS $address)
                                            <option value="<?php echo $address->address.','.$address->city.','.$address->state.','.$address->zip; ?>" selected><?php echo $address->address.','.$address->city.','.$address->state.','.$address->zip; ?></option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="phone">Phone</label><a href="javascript:void(0);" id="add-more" data-type="phone" class="float-right">Add
                                    More</a>
                                @if(isset($phones) && count($phones) > 0)
                                    ...
                                    @foreach($phones AS $phone)
                                        ...
                                        @endphp
                                        <input class="form-control {{ $p > 0 ? 'mt-2' : '' }}" name="{{$phone_name}}" type="text" id="phone" placeholder="Enter your phone" required
                                               value="{{ $phone->phone ?? old('phone')}}"/>
                                        @php $p++; @endphp
                                    @endforeach
                                @else
                                    <input class="form-control" name="phone" type="text" id="phone" placeholder="Enter your phone" required
                                           value="{{ old('phone')}}"/>
                                @endif
                            </div>
                            <div class="form-group">
                                ...
                                <input class="form-control  @if($errors->has('dob')) is-invalid @endif"
                                       ...
                                       value="{{ date('m/d/Y', strtotime($customer->dob)) ?? old('dob')}}" data-provide="datepicker"/>
                                @if($errors->has('dob'))
                                    <span class="invalid-feedback" role="alert">
                            ...
                            <div class="form-group">
                                ...
                                <input class="form-control @if($errors->has('ssn')) is-invalid @endif" name="ssn" type="text" required
                                       id="ssn" placeholder="Enter SSN" value="{{ $customer->ssn ?? old('ssn')}}"/>
                                @if($errors->has('ssn'))
                                    <span class="invalid-feedback" role="alert">
                            ...
                            <div class="form-group">
                                <label for="position">Positions</label><a href="javascript:void(0);" data-toggle="modal" data-target="#custom-modal" class="float-right">Add
                                    New Position</a>
                                @if($companies)
                                    <select class="form-control select2" name="position[]" id="position" data-toggle="select2" multiple>
                                        @foreach($companies AS $company)
                                            <option value="{{$company->id}}" {{ (isset($company_ids) && count($company_ids) && in_array($company->id, $company_ids)) ? 'selected="selected"' : '' }}>{{$company->name}}</option>
                                        @endforeach
                                    </select>
                                @endif
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                @if($accout_types)
                                    <select class="form-control" name="account_type" id="account_type">
                                        ...
                                        @foreach($accout_types AS $account_type)
                                            <option value="{{$account_type}}" {{ ($customer->account_type == $account_type) ? 'selected="selected"' : '' }}>{{$account_type}}</option>
                                        @endforeach
                                    </select>
                                @endif
                            </div>
                            <div class="form-group">
                                ...
                                <input class="form-control auc @if($errors->has('amount_per_person')) is-invalid @endif"
                                       ...
                                       value="{{ $customer->amount_per_person ?? old('amount_per_person')}}"/>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control textarea" cols="10" rows="3" name="notes" id="notes">{{ $customer->notes }}</textarea>
                            </div>
                            <!-- <div class="form-group">
                                <label for="requirement_date">Requirement Date</label>
                                <input class="form-control" name="requirement_date" type="text" id="requirement_date" placeholder="Pick Requirement Date"
                                       value="{{ date('m/d/Y', strtotime($customer->requirement_date)) ?? old('requirement_date')}}" data-provide="datepicker"/>
                            </div> -->
                            <div class="form-group">
                                ...
                                <input class="form-control @if($errors->has('representative')) is-invalid @endif"
                                       ...
                                       value="{{ $customer->representative ?? old('representative')}}"/>
                            ...
                            <div class="form-group mb-0 text-center">
                                <button class="btn btn-success btn-block" type="submit"> Update Customer</button>
                            </div>
                ...
                <div class="modal-body p-4">
                    <form action="javascript:void(0);" method="post">
                        ...
                        <div class="text-right">
                            <button type="submit" class="btn btn-success waves-effect waves-light save-company">Save
                            </button>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body p-4">
                    <form action="javascript:void(0);" method="post">
                        ...
                        <div class="text-right">
                            <button type="submit" class="btn btn-success waves-effect waves-light save-address">Save
                            </button>
                            <button type="button" class="btn btn-danger waves-effect waves-light" data-dismiss="modal">
                                Cancel
                            </button>
